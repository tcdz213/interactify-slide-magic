import { Eye, Pencil, DollarSign, ToggleLeft, ToggleRight, Trash2, ArrowRightLeft } from "lucide-react";
...
export function ProductTable({ items, isVisible, sortKey, sortDir, onSort, onView, onEdit, onPricing, onToggle, onDelete, onUnits }: ProductTableProps) {
  const { unitsOfMeasure, productUnitConversions, productBaseUnits } = useWMSData();

  // Pre-compute conversion counts and base unit labels
      ...
      <table className="w-full text-sm min-w-[700px]">
        <thead>
          <tr className="border-b border-border/50 bg-muted/30">
            ...
            {isVisible("actions") && <th className="px-4 py-3 text-center font-medium text-muted-foreground" style={{ minWidth: 180 }}>Actions</th>}
          </tr>
        </thead>
        <tbody>
          {items.map(p => {
              ...
              <tr key={p.id} className="border-b border-border/30 hover:bg-muted/20 transition-colors">
                ...
                {isVisible("conversions") && (
                  <td className="px-4 py-3 text-center">
                    {(meta?.convCount ?? 0) > 0 ? (
                      <span className="inline-flex items-center justify-center min-w-[24px] rounded-full px-2 py-0.5 text-xs font-semibold bg-accent text-accent-foreground">
                        {meta?.convCount}
                      </span>
                    ) : (
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex items-center justify-center gap-1">
                      ...
                      <RBACGuard permission="manage_products">
                        ...
                        <button onClick={() => onDelete(p)} className="p-1.5 rounded-md hover:bg-destructive/10 text-destructive" title="Archiver (soft delete)">
