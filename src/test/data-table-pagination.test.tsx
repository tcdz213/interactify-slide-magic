...
describe("DataTablePagination", () => {
  ...
  it("renders showing text", () => {
    ...
    const p = screen.getByText((_, el) => el?.tagName === "P" && /1.*–.*10.*sur.*25/.test(el.textContent || ""));
  ...
  it("calls onPageSizeChange when select changes", () => {
    ...
    // Radix Select uses a button with role="combobox", not a native select
  ...
  it("shows correct range on page 2", () => {
    ...
    const p = screen.getByText((_, el) => el?.tagName === "P" && /11.*–.*20.*sur.*25/.test(el.textContent || ""));
    expect(p).toBeInTheDocument();
  });
});
