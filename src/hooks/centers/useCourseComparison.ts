...
const comparisonSlice = createSlice({
  ...
  reducers: {
    addCourse: (state, action: PayloadAction<Center>) => {
...
export const { addCourse, removeCourse, clearCourses } = comparisonSlice.actions;
export const comparisonReducer = comparisonSlice.reducer;

export const useCourseComparison = () => {
  const { toast } = useToast();
  const dispatch = useDispatch();
  const compareCourses = useSelector((state: RootState) => (state as any).comparison?.courses || []);

  const addToComparison = (course: Center) => {
    if (compareCourses.some((c: Center) => c.id === course.id)) {
      toast({
  ...
  const isInComparison = (courseId: number) => {
    return compareCourses.some((course: Center) => course.id === courseId);
  };
