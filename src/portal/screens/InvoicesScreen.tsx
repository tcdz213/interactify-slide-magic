...
const FILTERS = ["Toutes", "En attente", "Payées", "Échues"] as const;
type Filter = (typeof FILTERS)[number];

const filterMatch = (status: string, f: Filter) => {
  if (f === "Toutes") return true;
  if (f === "En attente") return ["issued", "partially_paid"].includes(status);
  if (f === "Payées") return status === "paid";
  if (f === "Échues") return status === "overdue";
  return true;
};
  ...
  return (
    <div className="p-4 space-y-4 animate-fade-in">
        ...
        {FILTERS.map((f) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap border transition-colors ${
              filter === f ? "bg-primary text-primary-foreground border-primary" : "bg-card border-border text-muted-foreground hover:bg-muted"
            }`}
          >
            {f}
          </button>
      ...
      <div className="space-y-2">
          ...
          return (
            ...
            >
              ...
              <div className="flex justify-between text-[10px] text-muted-foreground">
                <span>Émise: {new Date(inv.issuedDate).toLocaleDateString("fr-FR")}</span>
              ...
              <div className="flex gap-2 pt-1">
                <button
                  onClick={() => {
                    ...
                    toast({ title: "📥 PDF téléchargé", description: `${inv.id}.pdf` });
                  }}
                ...
                <button
                  onClick={() => toast({ title: "📧 Email envoyé", description: `Facture ${inv.id} envoyée` })}
        ...
        {filtered.length === 0 && (
          <div className="text-center py-8 text-sm text-muted-foreground">Aucune facture</div>
        )}
      </div>
    </div>
  );
