...
export default function SupplierReturnsPage() {
  ...
  const approveReturn = (id: string) => {
    ...
    toast({ title: t("supplierReturns.returnApproved"), description: id });
  };
  ...
  const processReturn = (id: string) => {
    ...
    if (ret.type === "Vendor") {
      const journal = generateSupplierReturnJournal({ id: ret.id, partyName: ret.partyName, totalValue: ret.totalValue, creditNoteId: linkedCN?.id });
      toast({ title: t("supplierReturns.vendorReturnProcessed"), description: `${id} — Journal ${journal.id} (${currency(journal.totalAmount)})` });
    } else {
      const journal = generateCustomerReturnJournal({ id: ret.id, partyName: ret.partyName, totalValue: ret.totalValue, creditNoteId: linkedCN?.id });
      toast({ title: t("supplierReturns.customerReturnProcessed"), description: `${id} — Journal ${journal.id} (${currency(journal.totalAmount)})` });
    }
    ...
    <div className="glass-card rounded-xl overflow-hidden">
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
          ...
          <p className="font-medium">{t("supplierReturns.noReturns")}</p>
        </div>
      ) : (
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground">{t("supplierReturns.colId")}</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground">{t("supplierReturns.colType")}</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground">{t("supplierReturns.colParty")}</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground">{t("supplierReturns.colReference")}</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground">{t("supplierReturns.colReason")}</th>
              <th className="px-4 py-3 text-right text-xs font-medium text-muted-foreground">{t("supplierReturns.colValue")}</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground">{t("supplierReturns.colStatus")}</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground">{t("supplierReturns.colActions")}</th>
            </tr>
          </thead>
          <tbody>
            {items.map((r: any) => (
              <tr key={r.id} className="border-b border-border/40 hover:bg-muted/20 transition-colors">
                ...
                <td className="px-4 py-3 text-xs">
                  <span className={`px-2 py-0.5 rounded text-[10px] font-medium ${r.type === "Customer" ? "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400" : "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"}`}>
                    {r.type === "Customer" ? t("supplierReturns.typeCustomer") : t("supplierReturns.typeVendor")}
                  </span>
                </td>
                  ...
                  <div className="flex gap-1">
                    {r.status === "Pending_QC" && (
                      <Button variant="outline" size="sm" onClick={() => approveReturn(r.id)}>
                        <CheckCircle2 className="h-3 w-3 mr-1" /> {t("supplierReturns.approve")}
                      </Button>
                    )}
                    {r.status === "Approved" && (
                      <Button variant="outline" size="sm" onClick={() => processReturn(r.id)}>
                        <Package className="h-3 w-3 mr-1" /> {t("supplierReturns.process")}
                      </Button>
                    )}
                    <Button variant="ghost" size="sm" onClick={() => showChain(r)} aria-label={t("supplierReturns.accountingChain", { id: r.id })}>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold tracking-tight">{t("supplierReturns.title")}</h1>
          <p className="text-sm text-muted-foreground">{t("supplierReturns.subtitle")}</p>
        </div>
      </div>

      <div className="grid grid-cols-4 gap-3">
        <div className="glass-card rounded-lg p-3 border border-border/50">
          <p className="text-xs text-muted-foreground uppercase tracking-wider">{t("supplierReturns.totalReturns")}</p>
          <p className="text-xl font-semibold">{stats.total}</p>
        </div>
        <div className="glass-card rounded-lg p-3 border border-border/50">
          <p className="text-xs text-muted-foreground uppercase tracking-wider">{t("supplierReturns.pending")}</p>
          <p className="text-xl font-semibold text-warning">{stats.pending}</p>
        </div>
        <div className="glass-card rounded-lg p-3 border border-border/50">
          <p className="text-xs text-muted-foreground uppercase tracking-wider">{t("supplierReturns.processed")}</p>
          <p className="text-xl font-semibold text-success">{stats.processed}</p>
        </div>
        <div className="glass-card rounded-lg p-3 border border-border/50">
          <p className="text-xs text-muted-foreground uppercase tracking-wider">{t("supplierReturns.totalValue")}</p>
      ...
      <div className="relative max-w-xs">
        ...
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
          placeholder={t("supplierReturns.search")}
          className="h-9 w-full rounded-lg border border-input bg-background pl-9 pr-4 text-sm" />
      </div>

      <Tabs defaultValue="all">
        <TabsList>
          <TabsTrigger value="all">{t("supplierReturns.tabAll", { count: allReturns.length })}</TabsTrigger>
          <TabsTrigger value="customer">{t("supplierReturns.tabCustomer", { count: customerReturns.length })}</TabsTrigger>
          <TabsTrigger value="vendor">{t("supplierReturns.tabVendor", { count: vendorReturns.length })}</TabsTrigger>
        </TabsList>
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                  ...
                  {t("supplierReturns.accountingChain", { id: chainDialog.ret.id })}
                </DialogTitle>
              </DialogHeader>
              ...
              <div className="flex items-center gap-2 py-4 flex-wrap">
                ...
                {chainDialog.linkedCN ? (
                  ...
                  <Badge variant="outline" className="px-3 py-1.5 text-sm text-muted-foreground">
                    {t("supplierReturns.noCreditNote")}
                  </Badge>
                )}
                <ArrowRight className="h-4 w-4 text-muted-foreground" />
              ...
              <div className="rounded-lg border p-4 space-y-2">
                <h4 className="text-sm font-semibold flex items-center gap-2">
                  <RotateCcw className="h-4 w-4" /> {t("supplierReturns.returnSection")}
                </h4>
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div><span className="text-muted-foreground">{t("supplierReturns.type")}</span> {chainDialog.ret.type === "Customer" ? t("supplierReturns.typeCustomer") : t("supplierReturns.typeVendor")}</div>
                  <div><span className="text-muted-foreground">{t("supplierReturns.party")}</span> <span className="font-medium">{chainDialog.ret.partyName}</span></div>
                  <div><span className="text-muted-foreground">{t("supplierReturns.value")}</span> <span className="font-medium">{currency(chainDialog.ret.totalValue)}</span></div>
                  <div><span className="text-muted-foreground">{t("supplierReturns.status")}</span> <StatusBadge status={chainDialog.ret.status} /></div>
                </div>
              </div>
              ...
              {chainDialog.linkedCN && (
                <div className="rounded-lg border p-4 space-y-2 border-emerald-200 dark:border-emerald-800">
                  <h4 className="text-sm font-semibold flex items-center gap-2 text-emerald-700 dark:text-emerald-400">
                    <FileText className="h-4 w-4" /> {t("supplierReturns.creditNoteSection", { id: chainDialog.linkedCN.id })}
                  </h4>
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div><span className="text-muted-foreground">{t("supplierReturns.amountTTC")}</span> <span className="font-medium">{currency(chainDialog.linkedCN.totalAmount)}</span></div>
                    <div><span className="text-muted-foreground">{t("supplierReturns.status")}</span> <StatusBadge status={chainDialog.linkedCN.status} /></div>
                  </div>
                </div>
              )}
              ...
              {chainDialog.journals.length > 0 && chainDialog.journals.map((j: ReturnJournalEntry) => (
                <div key={j.id} className="rounded-lg border p-4 space-y-2 border-amber-200 dark:border-amber-800">
                  <h4 className="text-sm font-semibold flex items-center gap-2 text-amber-700 dark:text-amber-400">
                    <BookOpen className="h-4 w-4" /> {t("supplierReturns.journalSection", { id: j.id })}
                  </h4>
                  <p className="text-xs text-muted-foreground">{j.date} — {j.type === "SUPPLIER_RETURN" ? t("supplierReturns.supplierReturn") : t("supplierReturns.customerReturn")}</p>
                  <table className="w-full text-xs mt-2">
                    <thead><tr className="border-b border-border"><th className="text-left py-1">{t("supplierReturns.colAccount")}</th><th className="text-left py-1">{t("supplierReturns.colLabel")}</th><th className="text-right py-1">{t("supplierReturns.colDebit")}</th><th className="text-right py-1">{t("supplierReturns.colCredit")}</th></tr></thead>
                    <tbody>
                    ...
                    <tfoot>
                      <tr className="font-semibold border-t">
                        <td className="py-1" colSpan={2}>{t("supplierReturns.totalRow")}</td>
              ...
              {chainDialog.journals.length === 0 && chainDialog.ret.status !== "Processed" && (
                <div className="rounded-lg border border-dashed p-4 text-center text-sm text-muted-foreground">
                  {t("supplierReturns.journalAutoMessage")}
                </div>
              )}
            </>
          )}
