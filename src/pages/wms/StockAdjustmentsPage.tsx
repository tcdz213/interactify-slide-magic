...
export default function StockAdjustmentsPage() {
  ...
  const adjExportCols: ExportColumn<StockAdjustment>[] = [
    { key: "id", label: "ID" }, { key: "productName", label: "Produit" }, { key: "warehouseName", label: "Entrepôt" },
    { key: "type", label: "Type" }, { key: "direction", label: "Direction" }, { key: "qty", label: "Qté" },
    { key: "valueLoss", label: "Perte" }, { key: "status", label: "Statut" }, { key: "date", label: "Date" },
  ...
  // Visibility scope — what user can SEE
  const scopeFiltered = isFullAccess
    ? data
    : data.filter(a => accessibleWarehouseIds?.includes(a.warehouseId));
  ...
  const handleCreate = () => {
    if (!canOperateOn(newForm.warehouseId)) {
      toast({ title: "Accès refusé", description: "Vous n'avez pas l'autorité opérationnelle sur cet entrepôt.", variant: "destructive" });
      return;
    }
    ...
    toast({ title: "Ajustement créé", description: newAdj.id });
  };
  ...
  const handleAction = (id: string, action: "approve" | "reject") => {
    ...
    toast({ title: action === "approve" ? "Ajustement approuvé" : "Ajustement rejeté" });
  };
  ...
  const submitForApproval = (id: string) => {
    ...
    toast({ title: "Soumis pour approbation" });
  };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight">Ajustements de stock</h1>
            <p className="text-sm text-muted-foreground">{scopeFiltered.length} ajustements — Perte totale : {currency(scopeFiltered.filter(a => a.status === "Approved").reduce((s, a) => s + a.valueLoss, 0))}</p>
          </div>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => setExportOpen(true)} className="gap-2"><Download className="h-4 w-4" /> Exporter</Button>
          {canCreate ? (
            <Button onClick={() => setShowCreate(true)} className="gap-2"><Plus className="h-4 w-4" /> Nouvel ajustement</Button>
          ) : (
            <Button disabled className="gap-2 opacity-50"><Lock className="h-4 w-4" /> Nouvel ajustement</Button>
          )}
        </div>
      </div>

      <div className="flex gap-2">
        {["all", "Damage", "Expiry", "Correction", "Theft", "Shrinkage", "Found"].map(t => (
          <button key={t} onClick={() => setFilterType(t)}
            ...
            {t === "all" ? "Tous" : t === "Damage" ? "Dommage" : t === "Expiry" ? "Expiration" : t === "Correction" ? "Correction" : t === "Theft" ? "Vol" : t === "Shrinkage" ? "Perte" : "Trouvé"}
          </button>
        ))}
      </div>
      ...
      <div className="glass-card rounded-xl overflow-hidden">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center text-muted-foreground">
            ...
            <p className="text-sm mt-1">Ajustez les filtres ou créez un nouvel ajustement.</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                ...
                <th className="text-center px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">Direction</th>
                <th className="text-right px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">Qty</th>
                <th className="text-right px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">Perte</th>
                <th className="text-left px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">Approbation</th>
            ...
            <tbody>
              {filtered.map(adj => {
                ...
                const variancePct = invItem && invItem.qtyOnHand > 0
                  ? (adj.qty / invItem.qtyOnHand) * 100
                  : null;
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        {adj.status === "Draft" && userCanAct && (
                          <button onClick={() => submitForApproval(adj.id)} className="p-1.5 rounded-md hover:bg-info/10" title="Soumettre"><CheckCircle className="h-3.5 w-3.5 text-info" /></button>
                        )}
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              {(["Draft", "Pending_Approval", "Approved", "Rejected"] as const).filter(s => s !== adj.status).map(s => (
                                <DropdownMenuItem key={s} onClick={() => { setData(prev => prev.map(a => a.id === adj.id ? { ...a, status: s } : a)); toast({ title: "Statut modifié", description: `${adj.id} → ${s}` }); }}>
          ...
          {selectedAdj && (
            <>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm mt-4">
                <div><span className="text-muted-foreground">Produit :</span> <span className="font-medium">{selectedAdj.productName}</span></div>
                <div><span className="text-muted-foreground">Emplacement :</span> <span className="font-mono">{selectedAdj.locationId}</span></div>
                <div><span className="text-muted-foreground">Type :</span> {selectedAdj.type}</div>
                <div><span className="text-muted-foreground">Direction :</span> {selectedAdj.direction === "Increase" ? "↑ Augmentation" : "↓ Diminution"}</div>
                <div><span className="text-muted-foreground">Quantité :</span> <span className="font-medium">{selectedAdj.qty}</span></div>
                <div><span className="text-muted-foreground">Perte :</span> {selectedAdj.valueLoss > 0 ? <span className="text-destructive font-medium">{currency(selectedAdj.valueLoss)}</span> : "—"}</div>
                <div><span className="text-muted-foreground">Créé par :</span> {selectedAdj.createdBy}</div>
                <div><span className="text-muted-foreground">Approuvé par :</span> {selectedAdj.approvedBy || "—"}</div>
                <div className="col-span-2"><span className="text-muted-foreground">Raison :</span> {selectedAdj.reason}</div>
                {selectedAdj.cycleCountRef && <div className="col-span-2"><span className="text-muted-foreground">Réf. inventaire :</span> <span className="font-mono">{selectedAdj.cycleCountRef}</span></div>}
              </div>
            </>
          )}
        </DialogContent>
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-5 mt-2">
            <FormSection title="Localisation" icon={<MapPin className="h-3.5 w-3.5" />}>
              <FormField label="Entrepôt" icon={<Package className="h-3.5 w-3.5" />} required hint="Seuls vos entrepôts opérationnels sont affichés">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <FormField label="Emplacement" hint="Optionnel — défaut si vide">
                  <select value={newForm.locationId} onChange={e => setNewForm({ ...newForm, locationId: e.target.value })}
                    className={formSelectClass}>
                    <option value="">Défaut (premier dispo)</option>
            ...
            <FormSection title="Détails de l'ajustement" icon={<AlertTriangle className="h-3.5 w-3.5" />}>
              <div className="grid grid-cols-3 gap-3">
                <FormField label="Type" required>
                  <select value={newForm.type} onChange={e => setNewForm({ ...newForm, type: e.target.value as any })}
                    ...
                    <option value="Shrinkage">Perte</option>
              ...
              {/* R3: Unit selector for adjustments */}
              {newForm.productId && (() => {
                ...
                return (
                  <FormField label="Unité de saisie" hint="La quantité sera convertie en unité de base">
                    <select
                    ...
                    {sel && !sel.isStockUnit && baseEquiv > 0 && (
                      <p className="text-[10px] text-muted-foreground mt-0.5">= {baseEquiv.toLocaleString("fr-FR")} {getBaseUnitAbbr(newForm.productId)} en stock</p>
                    )}
                  </FormField>
                );
              })()}
              {newForm.productId && newForm.qty > 0 && newForm.direction === "Decrease" && (
                <div className="flex items-center gap-2 rounded-lg bg-destructive/5 border border-destructive/20 px-3 py-2">
                  ...
                  <p className="text-xs text-destructive">Perte estimée : <strong>{currency(Math.round(toBaseUnits(newForm.qty, newForm.unitFactor)) * (products.find(p => p.id === newForm.productId)?.unitCost ?? 0))}</strong></p>
                </div>
              )}
            </FormSection>

            <FormSection title="Justification" icon={<MessageSquare className="h-3.5 w-3.5" />}>
              <FormField label="Raison" required hint="Décrivez la cause et les circonstances de l'ajustement">
                <textarea value={newForm.reason} onChange={e => setNewForm({ ...newForm, reason: e.target.value })}
                  placeholder="Ex : Lot B2026-0115 expiré, constaté lors du contrôle zone A..."
          ...
          <DialogFooter className="mt-2">
            ...
            <Button onClick={handleCreate} disabled={!newForm.productId || !newForm.qty || !newForm.reason} className="gap-1.5">
              <Plus className="h-4 w-4" /> Créer l'ajustement
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
