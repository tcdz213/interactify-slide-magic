...
export function GrnPage() {
  ...
  const handleCreateGrn = (asDraft: boolean) => {
    ...
    if (editingGrn) {
      ...
      toast({ title: t('grnPage.draftSaved'), description: updated.id });
      return;
    }
    ...
    toast({ title: asDraft ? t('grnPage.grnSavedAsDraft') : t('grnPage.grnCreated'), description: newGrn.id });
  };
  ...
  const handleDeleteDraft = () => {
    if (deleteConfirm) {
      ...
      toast({ title: t('grnPage.draftDeleted'), description: deleteConfirm.id });
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight">{t('grnPage.title')}</h1>
            <p className="text-sm text-muted-foreground">{t('grnPage.subtitle')}</p>
          </div>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => setExportOpen(true)} className="gap-2"><Download className="h-4 w-4" /> {t('grnPage.export')}</Button>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        <div className="glass-card rounded-lg p-3 border border-border/50">
          <p className="text-xs text-muted-foreground uppercase tracking-wider">{t('grnPage.qcPending')}</p>
          <p className="text-xl font-semibold text-warning">{stats.qcPending}</p>
        </div>
        <div className="glass-card rounded-lg p-3 border border-border/50">
          <p className="text-xs text-muted-foreground uppercase tracking-wider">{t('grnPage.approvalPending')}</p>
          <p className="text-xl font-semibold text-info">{stats.approvalPending}</p>
        </div>
        <div className="glass-card rounded-lg p-3 border border-border/50">
          <p className="text-xs text-muted-foreground uppercase tracking-wider">{t('grnPage.approved')}</p>
          <p className="text-xl font-semibold text-success">{stats.approved}</p>
        </div>
        <div className="glass-card rounded-lg p-3 border border-border/50">
          <p className="text-xs text-muted-foreground uppercase tracking-wider">{t('grnPage.rejected')}</p>
          <p className="text-xl font-semibold text-destructive">{stats.rejected}</p>
        </div>
        <div className="glass-card rounded-lg p-3 border border-border/50">
          <p className="text-xs text-muted-foreground uppercase tracking-wider">{t('grnPage.drafts')}</p>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-xs">
          ...
          <input
            type="text"
            placeholder={t('grnPage.searchPlaceholder')}
            value={search}
        ...
        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="h-9 rounded-lg border border-input bg-muted/50 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring/20">
          <option value="all">{t('grnPage.allStatuses')}</option>
          <option value="Draft">{t('grnPage.statusDraft')}</option>
          <option value="QC_Pending">{t('grnPage.statusQcPending')}</option>
          <option value="Approval_Pending">{t('grnPage.statusApproval')}</option>
          <option value="Approved">{t('grnPage.statusApproved')}</option>
          <option value="Rejected">{t('grnPage.statusRejected')}</option>
        </select>
        <select value={sortBy} onChange={e => setSortBy(e.target.value as typeof sortBy)} className="h-9 rounded-lg border border-input bg-muted/50 px-3 text-sm">
          <option value="date">{t('grnPage.sortDate')}</option>
          <option value="vendor">{t('grnPage.sortVendor')}</option>
          <option value="value">{t('grnPage.sortValue')}</option>
          <option value="status">{t('grnPage.sortStatus')}</option>
        </select>
      ...
      <div className="glass-card rounded-xl overflow-x-auto">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center text-muted-foreground">
            ...
            <p className="font-medium">{t('grnPage.noGrn')}</p>
            <p className="text-sm mt-1">{t('grnPage.noGrnDesc')}</p>
          </div>
        ) : (
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              <th className="text-left px-4 py-3 font-medium text-muted-foreground text-xs uppercase tracking-wider">{t('grnPage.colGrn')}</th>
              <th className="text-left px-4 py-3 font-medium text-muted-foreground text-xs uppercase tracking-wider">{t('grnPage.colPo')}</th>
              <th className="text-left px-4 py-3 font-medium text-muted-foreground text-xs uppercase tracking-wider">{t('grnPage.colVendor')}</th>
              <th className="text-left px-4 py-3 font-medium text-muted-foreground text-xs uppercase tracking-wider">{t('grnPage.colDate')}</th>
              <th className="text-left px-4 py-3 font-medium text-muted-foreground text-xs uppercase tracking-wider">{t('grnPage.colItems')}</th>
              <th className="text-right px-4 py-3 font-medium text-muted-foreground text-xs uppercase tracking-wider">{t('grnPage.colValue')}</th>
              <th className="text-left px-4 py-3 font-medium text-muted-foreground text-xs uppercase tracking-wider">{t('grnPage.colStatus')}</th>
              <th className="text-right px-4 py-3 font-medium text-muted-foreground text-xs uppercase tracking-wider sticky right-0 bg-muted/30 min-w-[140px]">{t('grnPage.colActions')}</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((grn) => (
...
export function InventoryPage() {
  ...
  const INV_COLUMNS: import("@/components/ColumnToggle").ColumnDef[] = [
    { key: "sku", label: t("inventoryPage.colSku"), alwaysVisible: true },
    { key: "name", label: t("inventoryPage.colProduct"), alwaysVisible: true },
    { key: "category", label: t("inventoryPage.colCategory") },
    { key: "location", label: t("inventoryPage.colLocation") },
    { key: "available", label: t("inventoryPage.colAvailable") },
    { key: "onHand", label: t("inventoryPage.colOnHand") },
    { key: "reserved", label: t("inventoryPage.colReserved") },
    { key: "expiry", label: t("inventoryPage.colExpiry") },
    { key: "movement", label: t("inventoryPage.colMovement") },
  ];
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight">{t("inventoryPage.title")}</h1>
            <p className="text-sm text-muted-foreground">{t("inventoryPage.subtitle", { count: scopedInventory.length })}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { label: t("inventoryPage.totalStockValue"), value: currency(scopedInventory.reduce((s: number, i: any) => s + i.qtyOnHand * i.unitCostAvg, 0)) },
          { label: t("inventoryPage.belowMinLevel"), value: scopedInventory.filter((i: any) => i.qtyAvailable < i.minStockLevel).length + " " + t("inventoryPage.items"), alert: true },
          { label: t("inventoryPage.expiringItems"), value: scopedInventory.filter((i: any) => i.daysToExpiry <= 30).length + " " + t("inventoryPage.items"), alert: true },
          { label: t("inventoryPage.deadStock"), value: scopedInventory.filter((i: any) => i.daysSinceMovement > 90).length + " " + t("inventoryPage.items") },
        ].map((card) => (
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-xs">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <input type="text" placeholder={t("inventoryPage.searchPlaceholder")} value={search} onChange={(e) => setSearch(e.target.value)}
            className="h-9 w-full rounded-lg border border-input bg-muted/50 pl-9 pr-4 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring/20" />
        </div>
        ...
        <select value={filterWarehouse} onChange={(e) => setFilterWarehouse(e.target.value)}
          className="h-9 rounded-lg border border-input bg-muted/50 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring/20">
          <option value="all">{t("inventoryPage.allWarehouses")}</option>
          {warehouses.map((w: any) => <option key={w.id} value={w.id}>{w.name}</option>)}
        </select>
        <select value={filterStockLevel} onChange={(e) => setFilterStockLevel(e.target.value)}
          className="h-9 rounded-lg border border-input bg-muted/50 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring/20">
          <option value="all">{t("inventoryPage.allLevels")}</option>
          <option value="in_stock">{t("inventoryPage.inStock")}</option>
          <option value="low">{t("inventoryPage.lowStock")}</option>
          <option value="out">{t("inventoryPage.outOfStock")}</option>
        </select>
      </div>
      ...
      <div className="glass-card rounded-xl overflow-hidden">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center text-muted-foreground">
            ...
            <p className="font-medium">{t("inventoryPage.noItems")}</p>
            <p className="text-sm mt-1">{t("inventoryPage.noItemsDesc")}</p>
          </div>
        ) : (
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              {colVis.isVisible("sku") && <th className="text-left px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">{t("inventoryPage.colSku")}</th>}
              {colVis.isVisible("name") && <th onClick={() => toggleSort("name")} className="text-left px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground cursor-pointer hover:text-foreground">{t("inventoryPage.colProduct")} <SortIcon col="name" /></th>}
              {colVis.isVisible("category") && <th className="text-left px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">{t("inventoryPage.colCategory")}</th>}
              {colVis.isVisible("location") && <th className="text-left px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">{t("inventoryPage.colLocation")}</th>}
              {colVis.isVisible("available") && <th onClick={() => toggleSort("qty")} className="text-right px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground cursor-pointer hover:text-foreground">{t("inventoryPage.colAvailable")} <SortIcon col="qty" /></th>}
              {colVis.isVisible("onHand") && <th className="text-right px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">{t("inventoryPage.colOnHand")}</th>}
              {colVis.isVisible("reserved") && <th className="text-right px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">{t("inventoryPage.colReserved")}</th>}
              {colVis.isVisible("expiry") && <th onClick={() => toggleSort("expiry")} className="text-right px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground cursor-pointer hover:text-foreground">{t("inventoryPage.colExpiry")} <SortIcon col="expiry" /></th>}
              {colVis.isVisible("movement") && <th onClick={() => toggleSort("movement")} className="text-right px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground cursor-pointer hover:text-foreground">{t("inventoryPage.colMovement")} <SortIcon col="movement" /></th>}
            </tr>
          </thead>
          <tbody>
...
export function ReturnsPage() {
  ...
  const REASON_CODES: { value: string; label: string }[] = [
    { value: "DEFECTIVE", label: t("returnsPage2.reasonDefective") },
    { value: "DAMAGED", label: t("returnsPage2.reasonDamaged") },
    { value: "DAMAGED_DELIVERY", label: t("returnsPage2.reasonDamagedDelivery") },
    { value: "EXPIRED", label: t("returnsPage2.reasonExpired") },
    { value: "WRONG_ITEM", label: t("returnsPage2.reasonWrongItem") },
    { value: "WRONG_PRODUCT", label: t("returnsPage2.reasonWrongProduct") },
    { value: "WRONG_QTY", label: t("returnsPage2.reasonWrongQty") },
    { value: "QUALITY_FAIL", label: t("returnsPage2.reasonQualityFail") },
    { value: "QUALITY_COMPLAINT", label: t("returnsPage2.reasonQualityComplaint") },
    { value: "RECALL", label: t("returnsPage2.reasonRecall") },
    { value: "CONTRACT_BREACH", label: t("returnsPage2.reasonContractBreach") },
    { value: "CHANGE_OF_MIND", label: t("returnsPage2.reasonChangeOfMind") },
    { value: "WARRANTY", label: t("returnsPage2.reasonWarranty") },
    { value: "DUPLICATE_ORDER", label: t("returnsPage2.reasonDuplicateOrder") },
    { value: "OTHER", label: t("returnsPage2.reasonOther") },
  ];

  const DISPOSITION_CODES: { value: string; label: string }[] = [
    { value: "Restock", label: t("returnsPage2.dispRestock") },
    { value: "Restock_Discounted", label: t("returnsPage2.dispRestockDiscounted") },
    { value: "Scrap", label: t("returnsPage2.dispScrap") },
    { value: "Quarantine", label: t("returnsPage2.dispQuarantine") },
    { value: "Return_To_Vendor", label: t("returnsPage2.dispReturnToVendor") },
    { value: "Repair", label: t("returnsPage2.dispRepair") },
  ];
  ...
  const handleCreateReturn = (asDraft: boolean) => {
    ...
    toast({ title: asDraft ? t("returnsPage2.draftSaved") : t("returnsPage2.returnSubmitted"), description: newR.id });
  };
  ...
  const handleSubmit = (id: string) => {
    ...
    toast({ title: t("returnsPage2.returnSubmitted"), description: id });
  };
  ...
  const handleQCAction = (id: string, action: "approve" | "reject") => {
    ...
    toast({ title: action === "approve" ? t("returnsPage2.returnApproved") : t("returnsPage2.returnRejected"), description: id });
  };
  ...
  const handleShip = (id: string) => {
    ...
    toast({ title: t("returnsPage2.returnShipped"), description: id });
  };

  const handleProcess = (id: string) => {
    setData(prev => prev.map(r => r.id === id ? { ...r, status: "Processed" as const } : r));
    toast({ title: t("returnsPage2.returnProcessed"), description: id });
  };

  const handleCredit = (id: string) => {
    setData(prev => prev.map(r => r.id === id ? { ...r, status: "Credited" as const } : r));
    toast({ title: t("returnsPage2.returnCredited"), description: id });
  };

  const returnsExportCols = [
    { key: "id" as const, label: t("returnsPage2.colId") }, { key: "type" as const, label: t("returnsPage2.colType") }, { key: "referenceId" as const, label: t("returnsPage2.colRef") },
    { key: "partyName" as const, label: t("returnsPage2.colParty") }, { key: "date" as const, label: t("returnsPage2.colDate") }, { key: "reason" as const, label: t("returnsPage2.colReason") },
    { key: "totalValue" as const, label: t("returnsPage2.colValue") }, { key: "status" as const, label: t("returnsPage2.colStatus") },
  ];
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight">{t("returnsPage2.title")}</h1>
            <p className="text-sm text-muted-foreground">{t("returnsPage2.subtitle")}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button onClick={() => setShowCreate(true)} className="gap-2"><Plus className="h-4 w-4" /> {t("returnsPage2.newReturn")}</Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2">
        {[
          { label: t("returnsPage2.statusDraft"), count: statusCounts.draft, color: "text-muted-foreground" },
          { label: t("returnsPage2.statusSubmitted"), count: statusCounts.submitted, color: "text-info" },
          { label: t("returnsPage2.statusQC"), count: statusCounts.pendingQC, color: "text-warning" },
          { label: t("returnsPage2.statusApproved"), count: statusCounts.approved, color: "text-success" },
          { label: t("returnsPage2.statusShipped"), count: statusCounts.shipped, color: "text-info" },
          { label: t("returnsPage2.statusProcessed"), count: statusCounts.processed, color: "text-info" },
          { label: t("returnsPage2.statusCredited"), count: statusCounts.credited, color: "text-success" },
          { label: t("returnsPage2.statusRejected"), count: statusCounts.rejected, color: "text-destructive" },
        ].map(s => (
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-xs">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <input type="text" placeholder={t("returnsPage2.searchPlaceholder")} value={search} onChange={e => setSearch(e.target.value)}
            className="h-9 w-full rounded-lg border border-input bg-muted/50 pl-9 pr-4 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring/20" />
        </div>
        {["all", "Customer", "Vendor"].map((tp) => (
          <button key={tp} onClick={() => setFilterType(tp)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${filterType === tp ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground hover:text-foreground"}`}>
            {tp === "all" ? t("returnsPage2.allTypes") : tp === "Customer" ? t("returnsPage2.customers") : t("returnsPage2.vendors")}
          </button>
        ))}
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="h-9 rounded-lg border border-input bg-muted/50 px-3 text-sm">
          <option value="all">{t("returnsPage2.allStatuses")}</option>
          <option value="Draft">{t("returnsPage2.statusDraft")}</option>
          <option value="Submitted">{t("returnsPage2.statusSubmitted")}</option>
          <option value="Pending_QC">{t("returnsPage2.statusPendingQC")}</option>
          <option value="Approved">{t("returnsPage2.statusApproved")}</option>
          <option value="Shipped">{t("returnsPage2.statusShipped")}</option>
          <option value="Processed">{t("returnsPage2.statusProcessed")}</option>
          <option value="Credited">{t("returnsPage2.statusCredited")}</option>
          <option value="Rejected">{t("returnsPage2.statusRejected")}</option>
        </select>
      </div>
      ...
      <div className="glass-card rounded-xl overflow-hidden">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center text-muted-foreground">
            <Package className="h-12 w-12 mb-3 opacity-50" />
            <p className="font-medium">{t("returnsPage2.noReturns")}</p>
            <p className="text-sm mt-1">{t("returnsPage2.noReturnsDesc")}</p>
          </div>
        ) : (
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              <th className="text-left px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">{t("returnsPage2.colId")}</th>
              <th className="text-left px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">{t("returnsPage2.colType")}</th>
              <th className="text-left px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">{t("returnsPage2.colRef")}</th>
              <th className="text-left px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">{t("returnsPage2.colParty")}</th>
              <th className="text-left px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">{t("returnsPage2.colDate")}</th>
              <th className="text-left px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">{t("returnsPage2.colReason")}</th>
              <th className="text-right px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">{t("returnsPage2.colValue")}</th>
              <th className="text-left px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">{t("returnsPage2.colStatus")}</th>
              <th className="text-right px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">{t("returnsPage2.colActions")}</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((r) => (
              <tr key={r.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                ...
                <td className="px-4 py-3 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={() => setSelectedReturn(r)} className="p-1.5 rounded-md hover:bg-muted transition-colors" title={t("returnsPage2.viewDetails")}>
                      <Eye className="h-3.5 w-3.5 text-muted-foreground" />
                    </button>
                    {r.status === "Draft" && (
                      <button onClick={() => handleSubmit(r.id)} className="p-1.5 rounded-md hover:bg-info/10 transition-colors" title={t("returnsPage2.submit")}>
                        <Send className="h-3.5 w-3.5 text-info" />
                      </button>
                    )}
                    {(r.status === "Submitted" || r.status === "Pending_QC") && (
                      <>
                        <button onClick={() => handleQCAction(r.id, "approve")} className="p-1.5 rounded-md hover:bg-success/10 transition-colors" title={t("returnsPage2.approveQC")}>
                          <CheckCircle className="h-3.5 w-3.5 text-success" />
                        </button>
                        <button onClick={() => handleQCAction(r.id, "reject")} className="p-1.5 rounded-md hover:bg-destructive/10 transition-colors" title={t("returnsPage2.rejectQC")}>
                          <XCircle className="h-3.5 w-3.5 text-destructive" />
                        </button>
                      </>
                    )}
                    {r.status === "Approved" && (
                      <button onClick={() => handleShip(r.id)} className="p-1.5 rounded-md hover:bg-info/10 transition-colors" title={t("returnsPage2.ship")}>
                        <Send className="h-3.5 w-3.5 text-info" />
                      </button>
                    )}
                    {r.status === "Shipped" && (
                      <button onClick={() => handleProcess(r.id)} className="p-1.5 rounded-md hover:bg-success/10 transition-colors" title={t("returnsPage2.markProcessed")}>
                        <CheckCircle className="h-3.5 w-3.5 text-success" />
                      </button>
                    )}
                    {r.status === "Processed" && (
                      <button onClick={() => handleCredit(r.id)} className="p-1.5 rounded-md hover:bg-success/10 transition-colors" title={t("returnsPage2.credit")}>
                        <CheckCircle className="h-3.5 w-3.5 text-primary" />
                      </button>
                    )}
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <button className="p-1.5 rounded-md hover:bg-muted transition-colors" title={t("returnsPage2.changeStatus")}>
                          <RotateCcw className="h-3.5 w-3.5 text-muted-foreground" />
                        </button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end">
                        {(["Draft", "Submitted", "Pending_QC", "Approved", "Rejected", "Shipped", "Processed", "Credited"] as const).filter(s => s !== r.status).map(s => (
                          <DropdownMenuItem key={s} onClick={() => { setData(prev => prev.map(ret => ret.id === r.id ? { ...ret, status: s } : ret)); toast({ title: t("returnsPage2.statusChanged"), description: `${r.id} → ${s}` }); }}>
                            <StatusBadge status={s} />
                          </DropdownMenuItem>
                        ))}
                      </DropdownMenuContent>
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="max-w-xl max-h-[85vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{t("returnsPage2.dialogTitle")}</DialogTitle></DialogHeader>
          <div className="space-y-3 mt-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-sm font-medium text-muted-foreground">{t("returnsPage2.type")}</label>
                <select value={newRet.type} onChange={e => setNewRet(prev => ({ ...prev, type: e.target.value as "Customer" | "Vendor", refId: "", partyName: "" }))} className="mt-1 h-9 w-full rounded-lg border border-input bg-muted/50 px-3 text-sm">
                  <option value="Customer">{t("returnsPage2.typeCustomer")}</option>
                  <option value="Vendor">{t("returnsPage2.typeVendor")}</option>
                </select>
              </div>
              <div>
                <label className="text-sm font-medium text-muted-foreground">{t("returnsPage2.reference")}</label>
                <select value={newRet.refId} onChange={e => { const opt = refOptions.find(o => o.id === e.target.value); setNewRet(prev => ({ ...prev, refId: e.target.value, partyName: opt?.name || "" })); }} className="mt-1 h-9 w-full rounded-lg border border-input bg-muted/50 px-3 text-sm">
                  <option value="">{t("returnsPage2.selectRef")}</option>
                  {refOptions.map(o => <option key={o.id} value={o.id}>{o.id} — {o.name}</option>)}
                </select>
              </div>
            </div>
            <div>
              <label className="text-sm font-medium text-muted-foreground">{t("returnsPage2.party")}</label>
              <input value={newRet.partyName} onChange={e => setNewRet(prev => ({ ...prev, partyName: e.target.value }))} placeholder={t("returnsPage2.partyPlaceholder")} className="mt-1 h-9 w-full rounded-lg border border-input bg-muted/50 px-3 text-sm" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-sm font-medium text-muted-foreground">{t("returnsPage2.reasonCode")}</label>
              ...
              <div>
                <label className="text-sm font-medium text-muted-foreground">{t("returnsPage2.reasonText")}</label>
                <input value={newRet.reason} onChange={e => setNewRet(prev => ({ ...prev, reason: e.target.value }))} placeholder={t("returnsPage2.reasonPlaceholder")} className="mt-1 h-9 w-full rounded-lg border border-input bg-muted/50 px-3 text-sm" />
              </div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="text-sm font-medium text-muted-foreground">{t("returnsPage2.disposition")}</label>
                <select value={newRet.disposition} onChange={e => setNewRet(prev => ({ ...prev, disposition: e.target.value }))} className="mt-1 h-9 w-full rounded-lg border border-input bg-muted/50 px-3 text-sm">
                  <option value="">{t("returnsPage2.dispositionNone")}</option>
                  {DISPOSITION_CODES.map(d => <option key={d.value} value={d.value}>{d.label}</option>)}
                </select>
              </div>
              <div>
                <label className="text-sm font-medium text-muted-foreground">{t("returnsPage2.refundMethod")}</label>
                <select value={newRet.refundMethod} onChange={e => setNewRet(prev => ({ ...prev, refundMethod: e.target.value as any }))} className="mt-1 h-9 w-full rounded-lg border border-input bg-muted/50 px-3 text-sm">
                  <option value="Credit_Note">{t("returnsPage2.refundCreditNote")}</option>
                  <option value="Cash_Refund">{t("returnsPage2.refundCash")}</option>
                  <option value="Replacement">{t("returnsPage2.refundReplacement")}</option>
                  <option value="Store_Credit">{t("returnsPage2.refundStoreCredit")}</option>
                </select>
              </div>
              <div>
                <label className="text-sm font-medium text-muted-foreground">{t("returnsPage2.restockingFee")}</label>
            ...
            <div>
              <label className="text-sm font-medium text-muted-foreground">{t("returnsPage2.items")}</label>
              {newRet.items.map((it, i) => (
                <div key={i} className="flex gap-2 mt-1 mb-2 flex-wrap">
                  <select value={it.productId} onChange={e => setNewRet(prev => { const arr = [...prev.items]; arr[i] = { ...arr[i], productId: e.target.value, productName: products.find(p => p.id === e.target.value)?.name || "" }; return { ...prev, items: arr }; })} className="flex-1 min-w-[150px] h-9 rounded-lg border border-input bg-muted/50 px-3 text-sm">
                    <option value="">{t("returnsPage2.productPlaceholder")}</option>
                    {products.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                  <input type="number" placeholder={t("returnsPage2.qtyPlaceholder")} value={it.qty || ""} onChange={e => setNewRet(prev => { const arr = [...prev.items]; arr[i] = { ...arr[i], qty: Number(e.target.value) }; return { ...prev, items: arr }; })} className="w-16 h-9 rounded-lg border border-input px-2 text-sm" />
                  ...
                  <input placeholder={t("returnsPage2.reasonDetailPlaceholder")} value={it.reason} onChange={e => setNewRet(prev => { const arr = [...prev.items]; arr[i] = { ...arr[i], reason: e.target.value }; return { ...prev, items: arr }; })} className="flex-1 min-w-[120px] h-9 rounded-lg border border-input px-3 text-sm" />
              ...
              <Button variant="outline" size="sm" className="mt-1" onClick={() => setNewRet(prev => ({ ...prev, items: [...prev.items, { productId: "", productName: "", qty: 0, reason: "", reasonCode: "DEFECTIVE" }] }))}><Plus className="h-3 w-3 mr-1" /> {t("returnsPage2.addLine")}</Button>
            </div>
          </div>
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setShowCreate(false)}>{t("returnsPage2.cancel")}</Button>
            <Button variant="secondary" onClick={() => handleCreateReturn(true)} disabled={!newRet.refId || !newRet.partyName || !newRet.reason || !newRet.items.some(i => i.productId && i.qty > 0)}>{t("returnsPage2.saveDraft")}</Button>
            <Button onClick={() => handleCreateReturn(false)} disabled={!newRet.refId || !newRet.partyName || !newRet.reason || !newRet.items.some(i => i.productId && i.qty > 0)}>{t("returnsPage2.submitReturn")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          {selectedReturn && (
            <>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm mt-4">
                <div><span className="text-muted-foreground">{t("returnsPage2.detailType")}</span> <span className="font-medium">{selectedReturn.type === "Customer" ? t("returnsPage2.typeCustomer") : t("returnsPage2.typeVendor")}</span></div>
                <div><span className="text-muted-foreground">{t("returnsPage2.detailReference")}</span> <span className="font-mono">{selectedReturn.referenceId}</span></div>
                <div><span className="text-muted-foreground">{t("returnsPage2.detailParty")}</span> <span className="font-medium">{selectedReturn.partyName}</span></div>
                <div><span className="text-muted-foreground">{t("returnsPage2.detailDate")}</span> {selectedReturn.date}</div>
                <div><span className="text-muted-foreground">{t("returnsPage2.detailGrossValue")}</span> <span className="font-medium">{currency(selectedReturn.totalValue)}</span></div>
                {selectedReturn.netCredit !== undefined && <div><span className="text-muted-foreground">{t("returnsPage2.detailNetCredit")}</span> <span className="font-medium text-primary">{currency(selectedReturn.netCredit)}</span></div>}
                {selectedReturn.restockingFeePct ? <div><span className="text-muted-foreground">{t("returnsPage2.detailRestockingFee")}</span> {selectedReturn.restockingFeePct}%</div> : null}
                <div><span className="text-muted-foreground">{t("returnsPage2.detailProcessedBy")}</span> {selectedReturn.processedBy}</div>
                {selectedReturn.qcBy && <div><span className="text-muted-foreground">{t("returnsPage2.detailQCBy")}</span> {selectedReturn.qcBy}</div>}
                {selectedReturn.disposition && <div><span className="text-muted-foreground">{t("returnsPage2.detailDisposition")}</span> <StatusBadge status={selectedReturn.disposition} /></div>}
                {selectedReturn.refundMethod && <div><span className="text-muted-foreground">{t("returnsPage2.detailRefundMethod")}</span> {selectedReturn.refundMethod.replace(/_/g, " ")}</div>}
                {selectedReturn.creditNoteId && <div><span className="text-muted-foreground">{t("returnsPage2.detailCreditNote")}</span> <span className="font-mono text-primary">{selectedReturn.creditNoteId}</span></div>}
                {selectedReturn.reasonCode && <div><span className="text-muted-foreground">{t("returnsPage2.detailReasonCode")}</span> {selectedReturn.reasonCode}</div>}
                <div className="col-span-2"><span className="text-muted-foreground">{t("returnsPage2.detailReason")}</span> {selectedReturn.reason}</div>
                {selectedReturn.notes && <div className="col-span-2"><span className="text-muted-foreground">{t("returnsPage2.detailNotes")}</span> {selectedReturn.notes}</div>}
              </div>

              <div className="mt-4 flex items-center gap-2 flex-wrap">
                {selectedReturn.status === "Draft" && <Button size="sm" variant="outline" onClick={() => { handleSubmit(selectedReturn.id); setSelectedReturn(null); }}>{t("returnsPage2.detailSubmit")}</Button>}
                {(selectedReturn.status === "Submitted" || selectedReturn.status === "Pending_QC") && (
                  <>
                    <Button size="sm" variant="outline" className="text-success border-success/30" onClick={() => { handleQCAction(selectedReturn.id, "approve"); setSelectedReturn(null); }}>{t("returnsPage2.detailApproveQC")}</Button>
                    <Button size="sm" variant="outline" className="text-destructive border-destructive/30" onClick={() => { handleQCAction(selectedReturn.id, "reject"); setSelectedReturn(null); }}>{t("returnsPage2.detailRejectQC")}</Button>
                  </>
                )}
                {selectedReturn.status === "Approved" && <Button size="sm" onClick={() => { handleShip(selectedReturn.id); setSelectedReturn(null); }}>{t("returnsPage2.detailShip")}</Button>}
                {selectedReturn.status === "Shipped" && <Button size="sm" onClick={() => { handleProcess(selectedReturn.id); setSelectedReturn(null); }}>{t("returnsPage2.detailMarkProcessed")}</Button>}
                {selectedReturn.status === "Processed" && <Button size="sm" onClick={() => { handleCredit(selectedReturn.id); setSelectedReturn(null); }}>{t("returnsPage2.detailCredit")}</Button>}
              </div>

              <div className="mt-4">
                <h4 className="text-sm font-semibold mb-2">{t("returnsPage2.detailItems")}</h4>
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-border">
                      <th className="text-left py-2 px-2 text-muted-foreground">{t("returnsPage2.detailColProduct")}</th>
                      <th className="text-right py-2 px-2 text-muted-foreground">{t("returnsPage2.detailColQty")}</th>
                      <th className="text-right py-2 px-2 text-muted-foreground">{t("returnsPage2.detailColUnitCost")}</th>
                      <th className="text-right py-2 px-2 text-muted-foreground">{t("returnsPage2.detailColValue")}</th>
                      <th className="text-left py-2 px-2 text-muted-foreground">{t("returnsPage2.detailColReason")}</th>
                      <th className="text-left py-2 px-2 text-muted-foreground">{t("returnsPage2.detailColDisposition")}</th>
                      <th className="text-left py-2 px-2 text-muted-foreground">{t("returnsPage2.detailColQC")}</th>
                    </tr>
                  </thead>
                  <tbody>
