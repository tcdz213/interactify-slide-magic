...
const DOC_TYPE_LABELS: Record<CreditNoteType, string> = {
  Vendor_Credit: "Avoir fournisseur",
  Vendor_Debit: "Note de débit fournisseur",
  Customer_Credit: "Avoir client",
  Customer_Debit: "Note de débit client",
};
...
export default function CreditNotesPage() {
  ...
  const advanceStatus = (id: string, newStatus: CreditNoteStatus) => {
    ...
    toast({ title: `Statut → ${STATUS_LABELS[newStatus]}`, description: id });
    if (selected?.id === id) setSelected(null);
  };

  // Auto-generate credit note from a return
  const generateFromReturn = (returnId: string) => {
    ...
    if (existing) {
      toast({ title: "Avoir déjà existant", description: existing.id, variant: "destructive" });
      return;
    }
    ...
    toast({ title: "Avoir créé", description: `${newCN.id} — ${currency(newCN.totalAmount)}` });
  };

  // Find returns without credit notes for quick-generate
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight">Avoirs & Notes de débit</h1>
            <p className="text-sm text-muted-foreground">Credit notes, debit notes — génération automatique depuis les retours</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="glass-card rounded-lg p-3 border border-border/50">
          <p className="text-xs text-muted-foreground uppercase tracking-wider">Comptabilisés</p>
      ...
      {returnsWithoutCN.length > 0 && (
        <div className="glass-card rounded-lg p-3 border border-warning/30 bg-warning/5">
          <p className="text-sm font-medium mb-2">⚠️ {returnsWithoutCN.length} retour(s) sans avoir — Génération rapide :</p>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {(["all", "Customer_Credit", "Vendor_Credit", "Vendor_Debit", "Customer_Debit"] as const).map(t => (
          <button key={t} onClick={() => setFilterType(t)}
            ...
            {t === "all" ? "Tous" : DOC_TYPE_LABELS[t as CreditNoteType]}
          </button>
        ))}
      </div>
      ...
      <div className="glass-card rounded-xl overflow-hidden">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
            ...
            <p className="font-medium">Aucun avoir / note de débit</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                ...
                <th className="text-right px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">HT</th>
                <th className="text-right px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">TVA</th>
                <th className="text-right px-4 py-3 text-xs uppercase tracking-wider text-muted-foreground">TTC</th>
                ...
                <tr key={cn.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => setSelected(cn)} className="p-1.5 rounded-md hover:bg-muted transition-colors" title="Voir">
                        <Eye className="h-3.5 w-3.5 text-muted-foreground" />
                      </button>
                      {cn.status === "Draft" && (
                        <button onClick={() => advanceStatus(cn.id, "Validated")} className="p-1.5 rounded-md hover:bg-success/10 transition-colors" title="Valider">
                          <CheckCircle className="h-3.5 w-3.5 text-success" />
                        </button>
                      )}
                      {cn.status === "Validated" && (
                        <button onClick={() => advanceStatus(cn.id, "Posted")} className="p-1.5 rounded-md hover:bg-primary/10 transition-colors" title="Comptabiliser">
                          <FileText className="h-3.5 w-3.5 text-primary" />
                        </button>
                      )}
                      {cn.status === "Posted" && (
                        <button onClick={() => advanceStatus(cn.id, "Applied")} className="p-1.5 rounded-md hover:bg-success/10 transition-colors" title="Imputer">
                          <CheckCircle className="h-3.5 w-3.5 text-success" />
                        </button>
                      )}
                    </div>
          ...
          {selected && (
            <>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm mt-4">
                <div><span className="text-muted-foreground">Type :</span> <span className="font-medium">{DOC_TYPE_LABELS[selected.documentType]}</span></div>
                <div><span className="text-muted-foreground">Partie :</span> <span className="font-medium">{selected.partyName}</span></div>
                <div><span className="text-muted-foreground">Référence :</span> <span className="font-mono">{selected.referenceId}</span></div>
                <div><span className="text-muted-foreground">Date :</span> {selected.date}</div>
                <div><span className="text-muted-foreground">Sous-total HT :</span> <span className="font-medium">{currency(selected.subtotal)}</span></div>
                <div><span className="text-muted-foreground">TVA :</span> {currency(selected.taxAmount)}</div>
                <div><span className="text-muted-foreground">Total TTC :</span> <span className="font-bold text-primary">{currency(selected.totalAmount)}</span></div>
                <div><span className="text-muted-foreground">Créé par :</span> {selected.createdBy}</div>
                {selected.notes && <div className="col-span-2"><span className="text-muted-foreground">Notes :</span> {selected.notes}</div>}
              </div>

              {/* Journal preview */}
              <div className="mt-4 p-3 rounded-lg bg-muted/30 border border-border/50">
                <h4 className="text-sm font-semibold mb-2">📒 Écritures comptables prévisionnelles</h4>
                {selected.documentType === "Customer_Credit" ? (
                  <table className="w-full text-xs">
                    <thead><tr className="border-b border-border"><th className="text-left py-1">Compte</th><th className="text-right py-1">Débit</th><th className="text-right py-1">Crédit</th></tr></thead>
                    <tbody>
                  ...
                  <p className="text-xs text-muted-foreground">Écritures de note de débit — pénalité / ajustement</p>
                )}
              </div>

              <div className="mt-4">
                <h4 className="text-sm font-semibold mb-2">Lignes</h4>
                <table className="w-full text-xs">
                  <thead><tr className="border-b border-border"><th className="text-left py-2 px-2">Produit</th><th className="text-right py-2 px-2">Qté</th><th className="text-right py-2 px-2">P.U.</th><th className="text-right py-2 px-2">Total</th><th className="text-right py-2 px-2">TVA</th></tr></thead>
                  <tbody>
              ...
              <div className="mt-4 flex gap-2">
                {selected.status === "Draft" && <Button size="sm" onClick={() => advanceStatus(selected.id, "Validated")}>Valider</Button>}
                {selected.status === "Validated" && <Button size="sm" onClick={() => advanceStatus(selected.id, "Posted")}>Comptabiliser</Button>}
                {selected.status === "Posted" && <Button size="sm" onClick={() => advanceStatus(selected.id, "Applied")}>Imputer sur facture</Button>}
                {selected.status !== "Cancelled" && selected.status !== "Applied" && selected.status !== "Refunded" && (
                  <Button size="sm" variant="outline" className="text-destructive" onClick={() => advanceStatus(selected.id, "Cancelled")}>Annuler</Button>
                )}
              </div>
            </>
          )}
